...
function parse_flags(args) {
    const options = { include: [], exclude: [], update: false, dry: false };
    let target = '';
...
function lang_of(file) {
    const match = path.basename(file).match(/\.locale=([\w-]+)\.json$/);
    return match ? match[1] : null;
}
function source_files(target) {
    const stat = fs.statSync(target);
    if (stat.isFile())
        return lang_of(target) ? [target] : [];
 ...
 */
function dir_exists(root, rel) {
    let current = root;
    for (const segment of rel.split('/')) {
        if (!entries_of(current).includes(segment))
            return false;
...
function declared_keys(root, rel) {
...
function module_of(key, root) {
    const parts = key.replace(/^\$/, '').split('_');
    const candidates = [];
    for (let len = parts.length; len > 0; len--) {
        const dir = parts.slice(0, len).join('/');
        if (dir_exists(root, dir))
            candidates.push(dir);
    }
    for (const dir of candidates) {
        if (declared_keys(root, dir).has(key))
            return dir;
    }
    return candidates[0] ?? null;
}
function passes(dir, options) {
    if (options.exclude.some(frag => dir.includes(frag)))
        return false;
    if (options.include.length && !options.include.some(frag => dir.includes(frag)))
        return false;
    return true;
}
function locale(args) {
    ...
    if (unresolved.size) {
        ...
        for (const key of [...unresolved].sort().slice(0, 20))
            console.log(`  ${key}`);
        if (unresolved.size > 20)
            console.log(`  … и ещё ${unresolved.size - 20}`);
    }
}
