 ...
 */
function findDollarClasses(text) {
    const results = [];
...
function balancedBlock(text, openIndex) {
    ...
    if (stripped.length === text.length) {
        ...
        if (idx2 >= 0 && Math.abs(idx2 - openIndex) <= 5)
            sIdx = idx2;
    }
 ...
 */
function extractPropsFromClassBody(body) {
    const props = new Set();
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (depth === 1) {
            ...
            const name = matchMethodName(line) ||
        ...
        if (depth < 1)
            depth = 1; // avoid negative due to malformed input
    }
    // Exclude reserved
 ...
 */
function isAcceptablePropName(name) {
    return /^[a-z][\w]*$/.test(name) && !name.startsWith('_') && name !== 'constructor';
}
function matchMethodName(line) {
    // Examples:
    //   name( ... ) { ... }
    //   public name( ... ) { ... }
    //   protected name( ... ) { ... }
    ...
    //   async name( ... ) { ... }
    //   name<T>( ... ) { ... }
    const re = /^(?:\s*(?:public|protected|private|readonly|abstract|static|async)\s+)*\s*([a-z]\w*)\s*(?:<[^>]*>)?\s*\(/;
    const m = re.exec(line);
    return m ? m[1] : null;
}
function matchAccessorName(line) {
    // Examples:
    //   get name() { ... }
    //   set name(v: T) { ... }
...
function matchFieldName(line) {
    ...
    //   name = expr
    //   public name = ...
    const re = /^(?:\s*(?:public|protected|private|readonly|static)\s+)*\s*([a-z]\w*)\s*(?:[?!])?\s*(?::|=)\s*/;
    const m = re.exec(line);
    return m ? m[1] : null;
}
 ...
 */
function mergeTsPropsMaps(...maps) {
    const out = new Map();
    for (const m of maps) {
