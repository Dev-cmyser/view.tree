...
exports.sanitizeSeparators = sanitizeSeparators;
exports.sanitizeLineSpaces = sanitizeLineSpaces;
exports.spacingDiagnostics = spacingDiagnostics;
function formatText(text, _uri) {
...
function sanitizeSeparators(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const rest = m[2];
        if (rest.startsWith('\\'))
            continue; // raw string line: keep spaces
...
function sanitizeLineSpaces(line) {
    const m = /^([\t ]*)(.*)$/.exec(line);
    if (!m)
        return line;
    const indent = m[1];
...
function spacingDiagnostics(text) {
    const issues = [];
