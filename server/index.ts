...
documents.onDidChangeContent(async change => {
	...
	connection.sendDiagnostics({ uri, diagnostics: [...finalDiagnostics, ...style] })
	{
		...
		log(
			`[view.tree] diagnostics: uri=${uri} count=${diagnostics.length} index=classes:${stats.classes} props:${stats.props} occs:${stats.occs}`,
		)
	}
...
connection.onCompletion(params => {
	...
	log(
		`[view.tree] completion: uri=${uri} pos=${params.position.line}:${params.position.character} items=${items.length}`,
	)
	return items
})
...
connection.onHover(async params => {
	...
	log(
		`[view.tree] hover: uri=${uri} token=${token} defs=${defs.length} hasProps=${contents !== `view.tree: ${header}${value}`}`,
	)
	return hover
})
